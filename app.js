...
// 反馈弹窗总开关（管理员在"用户反馈"面板中设置，随云端状态同步给所有访客）
let feedbackModalEnabled = true;

function checkFeedbackModal(){
  if(!feedbackModalEnabled) return; // 管理员已在后台关闭反馈弹窗
  try{
...
function openFeedbackList() {
  ...
  if(toggle) toggle.checked = feedbackModalEnabled;
  switchFeedbackListTab('all');
  refreshFeedbackList();
}
async function onFeedbackEnabledToggle(checked) {
  feedbackModalEnabled = checked;
  try {
    await syncRemoteField('feedback_modal_enabled', checked);
    showToast(checked ? '已开启每日反馈弹窗' : '已关闭每日反馈弹窗');
  } catch (e) {
    console.error('反馈弹窗开关同步失败', e);
    showToast('保存失败，请检查网络后重试');
  }
}
...
function getSharedAppStateSnapshot() {
  return {
    ...
    feedback_modal_enabled: feedbackModalEnabled,
  };
}
...
function applyRemoteAppState(row) {
  ...
  if (row && row.feedback_modal_enabled !== undefined) {
    feedbackModalEnabled = !!row.feedback_modal_enabled;
  }
  updateLeaveBadge();
}
